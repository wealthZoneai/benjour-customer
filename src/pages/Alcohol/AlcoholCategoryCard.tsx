...
const AlcoholCategoryCard: React.FC<Props> = ({
  name,
  image,
  category,
  isFeatured = false,
  ...
  const handleClick = () => {
    navigate(`/alcohol/${category || name.toLowerCase()}`);
  };
      ...
      >
        <div className="relative h-full">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
            ...
            <span className="inline-flex items-center px-4 py-1.5 bg-amber-500 text-white text-sm font-medium rounded-full transition-all duration-300 transform group-hover:translate-x-2 group-hover:bg-amber-600">
              Explore Now
              <svg
  ...
  return (
    ...
    >
      ...
      <div className="p-5 text-center grow flex flex-col justify-center">
        <h2 className="text-lg font-semibold text-gray-800 group-hover:text-amber-600 transition-colors duration-300 mb-1">
          {name}
        </h2>

        <div className="flex items-center justify-center space-x-2 text-sm text-gray-500">
          <span>{Math.floor(Math.random() * 50) + 10} brands</span>
          <span>•</span>
...
export default AlcoholCategoryCard;
